# THIS PROGRAM FINDS UORF READS IN RIBOBASE GIVEN A CSV WITH UORF GENES, STARTS, AND STOP
#
...
def uorf_reads(database, transcripts):
    ...
    for study in study_dict.keys():
        study_path = ("STUDY_PATH/%s" % study) #EDIT
        current = current + 1
        if os.path.exists(study_path):
            os.chdir(study_path)
            for experiment in study_dict[study]:
                experiment_path = (study_path + "RIBOFILE_LOCATION/%s.ribo" % experiment) # EDIT
                            ...
                            for l in range(len(genes)):  # run through the gene names
                                ...
                                try:
                                    ...
                                    if (k == mmax):
                                        ...
                                        add_dic('Experiment Reads', exp_total.loc[gene][j], gene_reads)
...
## TO RUN:
write_output("STUDIES_DATABASE.csv", "INPUT.csv", "OUTPUT.csv", "PSITE_OFFSET.csv")
