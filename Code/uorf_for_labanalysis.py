...
def write_lab_output(transcripts, outfile) :
    ...
    genes = pd.read_csv(transcripts, sep=',', header=None, skiprows=1)[0].values.tolist()
    starts = pd.read_csv(transcripts, sep=',', header=None, skiprows=1)[1].values.tolist()
    stops = pd.read_csv(transcripts, sep=',', header=None, skiprows=1)[2].values.tolist()
...
# TO RUN:
write_lab_output("limbud_longest.csv", "OUTPUT.csv")